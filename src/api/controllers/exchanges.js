...
const orderBuy = async (req, res, next) => {
  try {
    ...
    const credenciais = await Configuracao.findOne({ 'usuario.id_usuario': params.id_usuario }).where({
...
const openOrders = async (req, res, next) => {
  try {
    ...
    ordens = await exchange.fetchOpenOrders((symbol = params.simbolo), (since = params.tempo), (limit = params.limite), (params = {}));
