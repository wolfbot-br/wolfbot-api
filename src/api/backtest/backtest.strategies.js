        ...
        tulind.indicators.macd.indicator([close], [short, long, signal], function (err, result) {
            if (err) {
                ...
                for (let i = 0; i <= close.length; i++) {
                    let time = moment(timestamp[i])
                    const digits = 4
                    let preco = parseFloat(close[i])
                    let macd = parseFloat(arrayMacd[i])
                    let sinal = parseFloat(arraySinal[i])
                    let histograma = parseFloat(arrayHistograma[i])
