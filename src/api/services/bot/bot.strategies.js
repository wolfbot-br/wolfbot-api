...
async function loadStrategy(config, params, target_currency, candle, ordersOpen) {
    const digits = 4;
    const { sellForIndicator } = config;
    const { profit } = config;
    const { stop } = config;
    const { maxOrdersOpen } = config;
    const timestamp = [];
    const open = [];
    const high = [];
    const low = [];
    ...
    if (config.strategy.indicators.stoch.status) {
        const { k_period } = config.strategy.indicators.stoch;
        const { k_slow_period } = config.strategy.indicators.stoch;
        const { d_period } = config.strategy.indicators.stoch;
    ...
    if (config.strategy.indicators.bbands.status) {
        const { period } = config.strategy.indicators.bbands;
